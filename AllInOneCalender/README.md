...
## Features
- Calendar-first home screen with a square day grid on a bright yellow background that shows up to five tasks directly in each date cell with tiny, elided labels plus a built-in [+] tile for quick adds, laid out with extra-compact spacing so one full month fits on a single mobile screen while the calendar stretches to fill the available viewport.
...
- Configure repeat rules directly in the composer: none, every day, every N days, or weekly across multiple weekdays, each with optional start/end dates and a one-tap bulk delete for the whole series when editing.
- Toggle the per-day previews independently via the app bar: show/hide icons, show/hide times, and show/hide titles to prioritize the information you need at a glance.
...
- The app bar shows the latest pull request creation time in a compact sub-label instead of a large version chip.
