...
## Features
- Calendar-first home screen with a square day grid on a bright yellow background that shows up to four tasks directly in each date cell plus a built-in [+] tile (five total slots) for quick adds, laid out to fit a full month on one mobile screen with compact typography.
